...
$bitexthai = new bitexthai('xxx','xxx','');//api key, secret, 2fA
$test = 'order';
switch($test){
	case 'order':
		//if($bitexthai->order(1, 'sell', 0.00078312 , 514000.000)){  // Currency Pairing ID, Buy/Sell, Amount, Rate
		//if($bitexthai->order(1, 'buy', 200 , 510000.000)){  // Currency Pairing ID, Buy/Sell, Amount, Rate
	...
	break;
	case 'cancel':
		if($bitexthai->cancel(1, 6955048)){ // Currency Pairing ID, Order ID
			echo 'Order Cancel';
		}else{
	...
	case 'history':
		if($history = $bitexthai->history()){
			foreach($history as $object) {
				echo $object->currency . " ". $object->amount. "<br/>";
			}
			print_r($history);
		}else{
			echo 'Failed: '.$bitexthai->msg;
	...
	break;
	case 'withdraw':
		if($withdraw = $bitexthai->withdraw('BTC',.59546, 'xxx')){ // Currency, Amount, Address
	...
	break;
	case 'bill_pay':
		if($billers = $bitexthai->billPayment(2,50,'xxx')){ // Topup Happy DTAC 50baht to phone 0868911218
			print_r($billers);
		}else{
			echo 'Failed: '.$bitexthai->msg;
		}
